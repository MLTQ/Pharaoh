...
- **Rationale**: Woosh and AudioLDM share the SFX server but have different setup paths. Native AudioLDM setup is the production path and expects `audioldm-m-full.ckpt` in the upstream cache; the Hugging Face command is retained only for the explicit diffusers fallback engine.

## Contracts

| Dependent | Expects | Breaking changes |
|-----------|---------|------------------|
| `sfx_server.py` | Native AudioLDM is installed with `PHARAOH_INSTALL_AUDIOLDM=1`; native checkpoints land in `~/.cache/audioldm`; diffusers fallback may use the HF local directory | Changing setup guidance without updating server resolution |
