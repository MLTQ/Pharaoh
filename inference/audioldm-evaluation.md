...
## Rationale
- AudioLDM v1 exposes duration control and upstream examples include long samples/soundscapes. The production integration should use the upstream `audioldm` runner and its recommended `audioldm-m-full` default rather than the deprecated diffusers pipeline or the smaller `audioldm-s-full-v2` checkpoint inside the Woosh environment.
...
- `audioldm`: explicit backend for long effects and ambience beds. Headless CLI routes `BED` rows and >5-second SFX rows here. It runs from `inference/.venv-audioldm` to avoid Woosh dependency conflicts.
