...
/// A single palette take file found on disk.
#[derive(serde::Serialize)]
pub struct PaletteTakeFile {
    pub path: String,
    pub sidecar: Option<SidecarMeta>,
}

/// Scan `characters/{character_id}/palette/` for all WAV files whose name starts
/// with `{emotion}_` and return them sorted by filename (oldest → newest).
/// Includes sidecar metadata when present. Used by the UI to surface takes
/// generated by MCP or other external tools that bypass the in-memory job store.
#[tauri::command]
pub fn list_palette_takes(
    app: AppHandle,
    project_id: String,
    character_id: String,
    emotion: String,
) -> Result<Vec<PaletteTakeFile>> {
    let palette_dir = app_projects_dir(&app)?
        .join(&project_id)
        .join("characters")
        .join(&character_id)
        .join("palette");
    ...
    let prefix = format!("{}_", emotion);
    let mut files: Vec<PaletteTakeFile> = std::fs::read_dir(&palette_dir)?
        .filter_map(|e| e.ok())
        .filter(|e| {
            let name = e.file_name().to_string_lossy().to_string();
        ...
        .map(|e| {
            ...
            let sidecar = std::fs::read_to_string(meta_path(&path))
                .ok()
                .and_then(|d| serde_json::from_str::<SidecarMeta>(&d).ok());
            PaletteTakeFile { path, sidecar }
        })
        .collect();
