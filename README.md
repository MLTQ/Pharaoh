...
- Spatial spaces: a 13-room catalog (vocal booth, bedroom, office, stairwell, hallway, small hall, concert hall, opera house, church, cathedral, mausoleum, cave, forest) — install via `./inference/download_spatial_assets.sh`, which tries documented FOSS sources first (OpenAir Library, Aachen AIR) and falls back to local DSP synthesis (`inference/synth_spatial_irs.py`) when a download fails or no URL is listed. Pick one per clip in the Spatialize modal; the renderer applies it via ffmpeg `afir` convolution. To upgrade any preset to a real measured IR, drop the WAV into `assets/spaces/` with the same filename.
