...
export function joinSession(opts = {}) {
  ...
  const apply = (key, value) => {
    state.set(key, value)
    subs.forEach((cb) => { try { cb(key, value) } catch {} })
  }
  ...
  const send = (key, value) => {
    apply(key, value) // local echo first — callers see their own writes immediately
    ...
    const connect = () => {
      ...
      ws.onmessage = (ev) => {
        ...
        if (msg.t === 'welcome') {
          ...
          opts.onPeers?.((msg.roster || []).length)
        } else if (msg.t === 'state') {
