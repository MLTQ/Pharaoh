...
const Wave = ({ width = 200, height = 24, seed = 1, color = "currentColor", count = 60, opacity = 0.85 }) => {
  // deterministic pseudo-wave
  const rng = (i) => {
  ...
  for (let i = 0; i < count; i++) {
    const env = Math.sin((i / count) * Math.PI) * 0.6 + 0.4; // hump
    const r = (rng(i) * 0.7 + 0.3) * env;
    const h = Math.max(1, r * height);
    bars.push(
      <rect key={i} x={i * bw + bw * 0.2} y={(height - h) / 2} width={bw * 0.55} height={h} fill={color} opacity={opacity} />
    );
  }
  return <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">{bars}</svg>;
};

const StatusRing = ({ s }) => <span className={`status-ring ${s}`} />;

const PlateTicks = ({ tts, sfx, music }) => (
  <div className="plate-ticks">
    <span className={`plate-tick tts ${tts ? "on" : ""}`} />
    <span className={`plate-tick sfx ${sfx ? "on" : ""}`} />
    <span className={`plate-tick music ${music ? "on" : ""}`} />
  </div>
);

Object.assign(window, { Icon, Wave, StatusRing, PlateTicks });
